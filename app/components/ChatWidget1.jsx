  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={openChat}
          className="fixed bottom-5 right-6 w-12 h-12 bg-[#854fff] text-white rounded-full flex items-center justify-center text-xl shadow-lg z-[1001] mb-24"
      ...
      {/* Chat Container */}
      {isOpen && (
        <div className="fixed bottom-5 right-5 w-[350px] h-[500px] bg-white rounded-xl shadow-lg flex flex-col z-50 overflow-hidden">
          {/* Header */}
...
const ChatWidget = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [
        ...prev, 
        { text: data.output || "Sorry, I couldn't understand that.", sender: 'bot' }
      ]);
    } catch (error) {
      console.error('Error:', error);
      setMessages(prev => [
  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={openChat}
          className="fixed bottom-5 right-5 w-12 h-12 bg-[#854fff] text-white rounded-full flex items-center justify-center text-xl shadow-lg z-50"
          aria-label="Open chat"
        >
          💬
        </button>
        ...
        <div className="fixed bottom-5 right-5 w-[350px] h-[500px] bg-white rounded-xl shadow-lg flex flex-col z-50 overflow-hidden">
          ...
          >
            {messages.map((msg, index) => (
              ...
              >
                {msg.sender === 'bot' && <strong>{msg.text}</strong>}
